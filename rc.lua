...
-- free: k_a, "r"; k_a, "m"
  ...
  awful.key(k_w, "c",      function () scratch.drop("copyq show", "center", "center", 0.5, 0.7 ) end),
  ...
  awful.key(k_a, "Print",  function () sexec("scrot -q 90 -bs "..home.."/.local/screenshots/w%H-%M-%S_%d.%m.%y.jpg") end),
  awful.key(k_n, "Print",  function () sexec("scrot -q 90 "..home.."/.local/screenshots/%H-%M-%S_%d.%m.%y.jpg") end),
  awful.key(k_w, "Print",  function () sexec("scrot -q 90 -t 30 "..home.."/.local/screenshots/t%H-%M-%S_%d.%m.%y.jpg") end),
...
if hostname == 'laptop' then
  globalkeys = awful.util.table.join(globalkeys,
    ...
    awful.key(k_n,  "XF86Battery",           function () sexec("sudo slimlockblank") end),
...
clientkeys = awful.util.table.join(
  ...
  awful.key(k_ac, "f",    function (c) -- toggle floating but avoid uneven window resolutions for streaming with ffmpeg
                            if twice then
  ...
  awful.key(k_ac, "m",    function ()  mywibox[mouse.screen].visible = not mywibox[mouse.screen].visible end),
  awful.key(k_ac, "o",    function (c) c.ontop = not c.ontop            end),
  awful.key(k_ac, "v",    function (c) c.minimized = not c.minimized    end),
  awful.key(k_as, "f",    awful.client.floating.toggle                     ),
...
awful.rules.rules = {
  ...
  { rule = { class = "Spotify", instance="spotify" },
    properties = { floating = true, tag = tags[1][5], geometry = { width = 1100, height = 884 } },
    callback = awful.placement.centered
  },
  { rule = { class = "Wine", instance="spotify.exe" },
    properties = { geometry = { width = 1100, height = 884 } },
    callback = awful.placement.centered
  },
  ...
  { rule_any = { class = { "Sonata", "Zenity", "Orage", "Plugin-container", "Gxmessage", "Keepnote" } },
    properties = { floating = true }
  },
  { rule = { class = "URxvt" },
    callback = awful.client.setslave
  },
  { rule = { instance = "urxvt_drop" },
    properties = { floating = true }
  },
  { rule_any = { class = { "Xephyr", "Plugin-container" } },
  ...
  { rule = { class = "Copyq" },
    ...
    callback = awful.placement.right
  },
  { rule = { class = "Zenity" },
    properties = { floating = true, sticky = true, ontop = true, above = true, geometry = { width = 500, height = 500, y=450, x=720 } },
    callback = awful.placement.centered
...
--TODO: toggle titlebars for floating windows
--client.add_signal(if floating.get(c) ~= nil then
